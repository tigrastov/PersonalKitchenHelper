...
//   // независимые формы добавления для каждого заказа
...
//     // очистка формы только для конкретного заказа
//     setAddForm((prev) => ({ ...prev, [orderId]: { category: "", recipeId: "", qty: "" } }));
//     loadAll();
//   };
...
//               <button className="delete-btn" onClick={() => handleDeleteOrder(o.id)}>
//                 ×
//               </button>
//             </div>
...
//               {o.items.map((item, i) => {
//                 const r = findRecipe(item.recipeId);
//                 return (
//                   <li key={i}>
//                     {r ? r.name : "???"} —{" "}
//                     <input
...
//                       min="1"
//                       value={item.qty}
//                       onChange={(e) => handleUpdateQty(o.id, i, e.target.value)}
//                     />{" "}
//                     шт.
...
//                 <li key={p.name}>
//                   {p.name} — {p.amount} {unitLabel(p.unit)}
//                 </li>
//               ))}
//             </ul>
...
//               {/* Выбор категории */}
//               <select
//                 value={addForm[o.id]?.category || ""}
//                 onChange={(e) => updateAddForm(o.id, "category", e.target.value)}
//               >
...
//               {/* Выбор блюда */}
//               <select
//                 value={addForm[o.id]?.recipeId || ""}
//                 onChange={(e) => updateAddForm(o.id, "recipeId", e.target.value)}
//               >
